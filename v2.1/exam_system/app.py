...
def check_compile(user_code):
    """检查代码是否能通过语法解析（编译）"""
    try:
        compile(user_code, '<student>', 'exec')
        return True, None
    except SyntaxError as e:
        return False, f"语法错误：第 {e.lineno} 行 - {e.msg}"

def check_keywords(user_code, keywords_str):
    """检查代码中是否包含指定关键字（逗号分隔）"""
    ...
    if not kws:
        return True, kws, kws
    found   = [k for k in kws if k in user_code]
    missing = [k for k in kws if k not in user_code]
...
def calc_coding_score(user_code, func_name, test_cases, num_random, time_limit,
                      keywords_str, total_score):
    """
    三段式评分：
      20% 编译通过
      20% 关键字命中（若无关键字配置则此部分并入测试用例分）
      60% 测试用例按比例得分
    返回 (earned, breakdown, sandbox_result)
    """
    has_kw_config = bool(keywords_str and keywords_str.strip())
    ...
    breakdown = {
        ...
        'keyword_found': [],
        'keyword_missing': [],
        'keywords_required': [k.strip() for k in (keywords_str or '').split(',') if k.strip()],
        'testcase_score': 0,
        'fixed_passed': 0,
        'fixed_total':  0,
    }
    ...
    # ── 1. 编译检查 ──────────────────────────────────────────
    compile_ok, compile_err = check_compile(user_code)
    breakdown['compile_ok']    = compile_ok
    breakdown['compile_err']   = compile_err
    breakdown['compile_score'] = round(total_score * compile_pct) if compile_ok else 0

    if not compile_ok:
        # 编译失败：后续均为 0 分
        breakdown['keyword_score']  = 0
        breakdown['testcase_score'] = 0
        return 0, breakdown, None

    # ── 2. 关键字检查 ────────────────────────────────────────
    if has_kw_config:
        kw_ok, kw_found, kw_missing = check_keywords(user_code, keywords_str)
        breakdown['keyword_ok']      = kw_ok
        breakdown['keyword_found']   = kw_found
        breakdown['keyword_missing'] = kw_missing
        breakdown['keyword_score']   = round(total_score * keyword_pct) if kw_ok else 0
    else:
        breakdown['keyword_ok']    = True
        breakdown['keyword_score'] = 0

    # ── 3. 测试用例 ──────────────────────────────────────────
    sandbox = run_sandbox(user_code, func_name, test_cases, num_random, time_limit)
    fixed_details = [d for d in sandbox['details'] if not d.get('is_random')]
    fixed_passed  = sum(1 for d in fixed_details if d.get('ok'))
    fixed_total   = len(fixed_details) if fixed_details else 0
    tc_ratio      = (fixed_passed / fixed_total) if fixed_total else 0
    tc_score      = round(total_score * testcase_pct * tc_ratio)

    breakdown['testcase_score'] = tc_score
    breakdown['fixed_passed']   = fixed_passed
    breakdown['fixed_total']    = fixed_total
    sandbox['fixed_passed']     = fixed_passed
    sandbox['fixed_total']      = fixed_total

    earned = breakdown['compile_score'] + breakdown['keyword_score'] + tc_score
    earned = min(earned, total_score)   # cap at full score
    return earned, breakdown, sandbox
...
def submit_exam(rid):
    ...
    for q in qs:
        ...
        if q['qtype'] == 'coding':
            ...
            keywords = q['keywords'] if q.get('keywords') else ''
            earned, breakdown, res = calc_coding_score(
                code, q['func_name'], tc, q['num_random'] or 5,
                q['time_limit'] or 5, keywords, q['score']
            )
            score += earned
            detail[qid]={'type':'coding','user':code,'ok':earned>=q['score'],
                         'score':q['score'],'earned':earned,
                         'sandbox':res,'breakdown':breakdown}
        elif q['qtype'] == 'multi':
...
def admin_question_edit(exam_id, qid=None):
    ...
    if request.method == 'POST':
        ...
        if qtype == 'coding':
            ...
            fields = (qtype,content,None,'',exp,sc,fn,sig,json.dumps(tc),nr,tl,kw)
            if qid:
                db.execute("UPDATE questions SET qtype=?,content=?,options=?,answer=?,explanation=?,"
                           "score=?,func_name=?,func_signature=?,test_cases=?,num_random=?,time_limit=?,keywords=? WHERE id=?",
                           fields+(qid,))
            else:
                ...
                db.execute("INSERT INTO questions(qtype,content,options,answer,explanation,score,"
                           "func_name,func_signature,test_cases,num_random,time_limit,keywords,exam_id,sort_order) "
